...
aegis_lookup_stmv = function( p, DS="all", locsmap=NULL, locs=NULL, timestamp=NULL, varnames=NULL, DB=NULL, ret="mean" ) {
  ...
  if (0) {
    ...
    # for space-time(year)
    sn = aegis_lookup_stmv( p=p, DS="spatial.annual", locsmap=locsmap, timestamp=set[,"timestamp"], varnames=newvars )
    names( sn  ) = newvars
    set = cbind( set,  sn )
  }
  ...
  if (DS=="all") {
    ...
    sn = NULL
    sn = aegis_lookup_stmv( p=p, DS="spatial", locsmap=locsmap, varnames=varnames )
    if (!is.null(sn)) {
      for (vn in names(sn) ) {
        if (is.list(sn)) {
          out[,vn] = sn[[vn]]
    ...
    sn = aegis_lookup_stmv( p=p, DS="spatial.annual", locsmap=locsmap, timestamp=timestamp, varnames=varnames )
    if (!is.null(sn)) {
      for (vn in names(sn) )  {
        if (is.list(sn)) {
          out[,vn] = sn[[vn]]
    ...
    if (length(vn_sts)>0) {
      sn = aegis_lookup_stmv( p=p, DS="spatial.annual.seasonal", locsmap=locsmap, timestamp=timestamp, varnames=vn_sts )
      if (!is.null(sn)) {
        for (vn in names(sn) )  {
          if (is.list(sn)) {
            out[,vn] = sn[[vn]]
    ...
    for (apds in aegis_project_datasources ) {
      ...
      if (length(pz_vars) > 0) {
        ...
        if (!is.null(PS)) {
          sn = aegis_lookup_stmv( p=pz, DS="spatial.annual", locsmap=locsmap, timestamp=timestamp,
