aegis_modelformula = function(p) {
    ...
    if (!exists("stmv_local_modelformula", p)) {
      if (p$stmv_local_modelengine =="twostep") {
        ...
        #   p$variables$Y, '~ s(yr, k=5, bs="ts") + s(cos.w, k=3, bs="ts") + s(sin.w, k=3, bs="ts") ',
        ...
        if (!exists("stmv_local_modelformula", p))  p$stmv_local_modelformula = formula( paste(
          p$variables$Y, ' ~ sx(yr,   bs="ps") + sx(cos.w, bs="ps") + s(sin.w, bs="ps") +s(z, bs="ps")',
